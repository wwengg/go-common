...
func (c *Client) OrderCallBack(data, mess, timestamp, sign string) (order OrderDetailInfo, err error) {
	...
	plaintext := fmt.Sprintf("data=%s&mess=%s&timestamp=%s&key=%s", data, mess, timestamp, c.Appkey)
	ok, err := VerifySign(plaintext, sign, c.YunPublicKey)
	if err != nil {
		log.Printf("@[%s] VerifySign failed, err=%v, data=%s, mess=%s, timestamp=%s, sign=%s", fn, err, data, mess, timestamp, sign)
		return
	}

	if !ok {
		log.Printf("@[%s] sign mismatch, data=%s, mess=%s, timestamp=%s, sign=%s", fn, data, mess, timestamp, sign)
