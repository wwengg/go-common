// @Title  
// @Description  
// @Author  Wangwengang  2021/5/25 下午1:56
// @Update  Wangwengang  2021/5/25 下午1:56
package main

import (
	"fmt"
	"github.com/wwengg/go-common/yunaccount"
)
...
func main() {
	// 初始化SDK客户端
	client := yunaccount.New(BrokerID, DealerID, Gateway, AppKey, Des3Key, PrivateKey, YunPublicKey)

	// 1、银行卡下单,响应云账户综合服务平台订单流水号
	bankOrderParam := &yunaccount.BankOrderParam{
	...
	aliOrderParam := &yunaccount.AliOrderParam{
	...
	wxOrderParam := &yunaccount.WxOrderParam{
		OpenID:    "aaabbbcccddd",
		Notes:     "微信打款备注",
	...
	userParam := &yunaccount.UserInfoParam{
		RealName:     "张三",
